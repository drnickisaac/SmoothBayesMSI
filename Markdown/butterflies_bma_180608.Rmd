...
```{r runButterflies, echo = FALSE, message = FALSE, fig.width = 6, fig.height = 4, warning = FALSE, results = 'hide'}
...
for(group in c('wider')){
    for(nk in c(8, 12, 16)){
#    for(nk in c(16)){
#        for(scaling_factor in  c(3)){
        for(scaling_factor in  c(1,3,9)){
...
#if(save_model) saveRDS(object=model, file=file.path('BMA_development/models/',
#                                    paste0(group,"_",nk,"_",scaling_factor,".rds")))
if(save_model) save(model, file=file.path(results_folder,
                                   paste0(group,"_",nk,"_",scaling_factor,".rData")))
